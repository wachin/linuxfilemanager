...
- [x] **T16** `invert_selection` ignora las casillas de selección: con `view.checkboxes` activo, los marcados por casilla permanecen en `_checked_paths` y siguen contando tras invertir (estado, copia, papelera). Tarea: que invertir/desmarcar sincronice también `checked_paths`. → **Resuelto (Fase 6.1, P2):** `invert_selection` ahora usa la raíz de la vista activa y, en modo casillas, reescribe `_checked_paths` como fuente de verdad; `select_all` también sincroniza las casillas. Tests en `tests/test_advanced_selection.py` (inversión con checks, y `show_selection_checkboxes` on/off).
