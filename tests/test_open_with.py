...
import lfm.utils.open_with as open_with_module
...
class OpenWithTests(unittest.TestCase):
    ...
    @patch("lfm.utils.open_with.subprocess.run")
    ...
    @patch("lfm.utils.open_with.os.environ", new_callable=dict)
    @patch("lfm.utils.open_with.subprocess.run")
    ...
    @patch("lfm.utils.open_with.subprocess.Popen")
    @patch("lfm.utils.open_with.shutil.which", return_value="/usr/bin/xdg-email")
    ...
    @patch("lfm.utils.open_with.shutil.which", return_value=None)
    def test_send_email_with_attachments_returns_false_without_xdg_email(self, _mock_which):
        result = open_with_module.send_email_with_attachments([self.path])

        self.assertFalse(result)

    @patch("lfm.utils.open_with.set_default_application", return_value=True)
    ...
    @patch("lfm.utils.open_with.set_default_application")
